...
function main() {
  ...
  const descriptorPath = path.join(__dirname, 'proto', 'todo_descriptor_set.bin');
  grpcServerReflection.addReflection(server, descriptorPath);

  const host = process.env.SERVER_HOST || "0.0.0.0";
  const port = process.env.GRPC_PORT || "50051";
  const bindAddress = `${host}:${port}`;
  server.bindAsync(bindAddress, grpc.ServerCredentials.createInsecure(), (err, boundPort) => {
    if (err) {
      console.error(err);
      return;
    }
